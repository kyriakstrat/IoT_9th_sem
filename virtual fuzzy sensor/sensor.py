...
def fuzzy_output(hour,input_season):
...
def plot_seasons():
    ...
    for i, hour in enumerate(time_of_day):
        for input_season, output_window_closed_all_hours in zip([3, 6, 9, 0],
            ...
            # Complement for window_closed
            output_window_closed_all_hours[i] = fuzzy_output(hour,input_season)
...
def calculate_propability_now():
    now = datetime.now()
    month = now.month
    year = now.year
    days_of_month = calendar.monthrange(year, month)[1]
    day = round(now.day/days_of_month-0.1,1)
    hour = now.hour + round(now.minute / 60-0.1,1)
    return(fuzzy_output(hour,month-1+day))

class VirtualWindowSensor:
    def __init__(self,name):
        self.name = name
        self.ind = None
        self.val = False
        self.topic = f'kyriakstrat/{self.name}'
        self.prob = None
        return
    
    def calculate_val(self):
        self.prob = calculate_propability_now()
        self.ind = random.random()
        # print(self.prob,ind)
    ...
    def helper(self):
        print(f'prob:{self.prob},ind:{self.ind}')
    def loop(self):
        time.sleep(5)
    ...
    def __str__(self) -> str:
        return f'sensor {self.name} with value {self.val}'
